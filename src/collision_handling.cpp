...
void handle_particle_particle_collision(Particle& p1, Particle& p2){
    ...
    if(line_of_contact.dot(p2_vel_along_loc - p1_vel_along_loc) >= 0){
        return;
    }
    ...
    p1.set_pos_vel(p1.get_curr_pos(), p1_vel_after_collision);
    p2.set_pos_vel(p2.get_curr_pos(), p2_vel_after_collision);
}